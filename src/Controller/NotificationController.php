<?php

namespace App\Controller;

use App\Entity\Notification;
use App\Entity\Evenement;
use App\Form\NotificationType;
use App\Repository\EvenementRepository;
use App\Repository\NotificationRepository;
use Doctrine\ORM\EntityManagerInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;
use Symfony\Component\HttpFoundation\JsonResponse;

#[Route('/notification')]
final class NotificationController extends AbstractController
{
    // Afficher toutes les notifications
    #[Route(name: 'app_notification_index', methods: ['GET'])]
    public function index(NotificationRepository $notificationRepository): Response
    {
        return $this->render('notification/index.html.twig', [
            'notifications' => $notificationRepository->findAll(),
        ]);
    }

    // Récupérer les notifications sous forme JSON
    #[Route('/notifications', name: 'app_notifications')]
    public function getNotifications(NotificationRepository $notificationRepository): JsonResponse
    {
        $notifications = $notificationRepository->findBy([], ['sentAt' => 'DESC']);
        $notificationData = [];

        foreach ($notifications as $notification) {
            $notificationData[] = [
                'id' => $notification->getId(),
                'titre' => $notification->getTitre(),
                'contenu' => $notification->getContenu(),
                'sentAt' => $notification->getSentAt()->format('Y-m-d H:i:s'),
            ];
        }

        return new JsonResponse($notificationData);
    }

    // Créer une nouvelle notification liée à un événement
    #[Route('/new/{id}', name: 'app_notification_new', methods: ['GET', 'POST'])]
    public function new(
        int $id,
        EvenementRepository $evenementRepository,
        Request $request,
        EntityManagerInterface $entityManager
    ): Response {
        // Récupérer l'événement par ID
        $evenement = $evenementRepository->find($id);
        if (!$evenement) {
            throw $this->createNotFoundException('Événement introuvable');
        }

        // Vérifier si l'événement a déjà une notification
        if ($evenement->getNotification()) {
            $this->addFlash('warning', 'Cet événement a déjà une notification.');
            return $this->redirectToRoute('app_notification_index');
        }

        // Création de la notification
        $notification = new Notification();
        $notification->setEvenement($evenement);

        // Formulaire
        $form = $this->createForm(NotificationType::class, $notification);
        $form->handleRequest($request);

        // Vérifier la soumission du formulaire
        if ($form->isSubmitted() && $form->isValid()) {
            $notification->setSentAt(new \DateTimeImmutable());
            $entityManager->persist($notification);
            $entityManager->flush();

            return $this->redirectToRoute('app_notification_index');
        }

        return $this->render('notification/new.html.twig', [
            'notification' => $notification,
            'form' => $form->createView(),
            'evenement' => $evenement,
        ]);
    }

    // Afficher une notification
    #[Route('/{id}', name: 'app_notification_show', methods: ['GET'])]
    public function show(Notification $notification): Response
    {
        return $this->render('notification/show.html.twig', [
            'notification' => $notification,
        ]);
    }

    // Afficher une notification pour le client
    #[Route('/notif/{id}', name: 'app_notification_show_cli', methods: ['GET'])]
    public function showClient(Notification $notification): Response
    {
        return $this->render('notification/showClient.html.twig', [
            'notification' => $notification,
        ]);
    }

    // Modifier une notification
    #[Route('/{id}/edit', name: 'app_notification_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, Notification $notification, EntityManagerInterface $entityManager): Response
    {
        $form = $this->createForm(NotificationType::class, $notification);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();
            return $this->redirectToRoute('app_notification_index');
        }

        return $this->render('notification/edit.html.twig', [
            'notification' => $notification,
            'form' => $form->createView(),
        ]);
    }

    // Supprimer une notification
    #[Route('/{id}', name: 'app_notification_delete', methods: ['POST'])]
    public function delete(Request $request, Notification $notification, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete' . $notification->getId(), $request->get('_token'))) {
            $entityManager->remove($notification);
            $entityManager->flush();
        }

        return $this->redirectToRoute('app_notification_index');
    }
}
